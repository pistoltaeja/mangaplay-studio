...
const mangaplayLintTheme = EditorView.baseTheme({
    ...
    // Inline emphasis — Fountain-style `*italic*`, `**bold**`, `***bold
    // italic***`, `_underline_`. Emitted by editor-inline-emphasis.js as
    // three stackable Decoration.mark classes plus a marker-hide widget
    // pair. `fontSize: 0` + `letterSpacing: 0` collapses the raw marker
    // characters visually without triggering the caret-jump behaviour
    // `display: none` produces on inline widget spans in CM6.
    ".cm-mp-bold": {
        fontWeight: "700"
    },
    ".cm-mp-italic": {

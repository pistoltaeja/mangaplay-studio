...
export class EditorSlotManager
{
     ...
     * @returns {EditorSlot}
     */
    openNew(path, text, format, fileUuid = null)
    {
        ...
        if (fileUuid) container.dataset.uuid = fileUuid;
     ...
     * @returns {EditorSlot}
     */
    replaceActive(path, text, format, fileUuid = null)
    {
        const slot = this.getActive();
        if (!slot)
        {
            return this.openNew(path, text, format, fileUuid);
        }
        ...
        if (fileUuid)
        {
            slot.container.dataset.uuid = fileUuid;
        }
        else
        {
            delete slot.container.dataset.uuid;
        }
     ...
     * @param {string|null} [newFileUuid] pass `undefined` (or omit) to leave
     *        the existing UUID untouched; pass `null` to clear it.
     */
    renamePath(tabId, newPath, newFileUuid = undefined)
    {
        ...
        if (newFileUuid !== undefined)
        {
            slot.fileUuid = newFileUuid;
            if (newFileUuid)
            {
                slot.container.dataset.uuid = newFileUuid;
            }
            else
            {
                delete slot.container.dataset.uuid;
     ...
     * @returns {{ openTabs: Array<{ id: string, path: string|null, fileUuid: string|null }>, activeTabId: string|null }}
     */
    serialize()
    {
        return {
            openTabs: this.slots.map((s) => ({ id: s.tabId, path: s.path, fileUuid: s.fileUuid ?? null })),
